...
{//cprintf ("%d mallox",size);
	virtual_address /= PAGE_SIZE;
	virtual_address *= PAGE_SIZE;
	uint32 req_pages = size/PAGE_SIZE + (size % PAGE_SIZE == 0 ? 0 : 1);
...
{
	//cprintf("H\n");
	uint32 req_pages = ROUNDUP(size,PAGE_SIZE);
	req_pages=req_pages/PAGE_SIZE;
	//cprintf ("%d \n",req_pages);
	//cprintf ("%d  s \n ",size);
	...
	for(int k=0;k<req_pages;k++){
		unsigned char *va = (unsigned char *)(ver) ;
	ver+=PAGE_SIZE;
	int count = 1024;
		uint32 *ptr_page;
		get_page_table(e->env_page_directory,(void*)ver,&ptr_page);
		if (ptr_page!=NULL){
