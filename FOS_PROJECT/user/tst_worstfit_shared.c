...
void _main(void)
{
	...
	{
		if(i==1)
			ptr_allocations[i]=smalloc("x",2*Mega,1);
		else if(i==7)
			ptr_allocations[i]=smalloc("y",2*Mega,1);
		else if(i==150)
			ptr_allocations[i]=smalloc("z",2*Mega,1);
		else
	...
	if( (sys_pf_calculate_allocated_pages() - usedDiskPages) !=  506*Mega/PAGE_SIZE) panic("Wrong page file allocation: ");
	if ((freeFrames - sys_calculate_free_frames()) != (6*Mega)/(PAGE_SIZE)+134) panic("Wrong allocation");
	...
	freeFrames = sys_calculate_free_frames() ;
	usedDiskPages = sys_pf_calculate_allocated_pages();
	tempAddress = smalloc("m",4 * Mega,1);			// Use Hole 4 -> Hole 4 = 5 M
	if((uint32)tempAddress != 0x8C900000)
		panic("Worst Fit not working correctly");
	if( (sys_pf_calculate_allocated_pages() - usedDiskPages) != 0) panic("Wrong page file allocation: ");
	if ((freeFrames - sys_calculate_free_frames()) != ( 4*Mega/PAGE_SIZE)+3) panic("Wrong allocation");
	cprintf("Test %d Passed \n", ++count);
	...
	freeFrames = sys_calculate_free_frames() ;
	usedDiskPages = sys_pf_calculate_allocated_pages();
	tempAddress = sget(envID,"m"); 			   // Use Hole 4 -> Hole 4 = 1 M
	...
	freeFrames = sys_calculate_free_frames() ;
	usedDiskPages = sys_pf_calculate_allocated_pages();
	tempAddress = sget(envID,"x"); 			// Use Hole 2 -> Hole 2 = 2 M
